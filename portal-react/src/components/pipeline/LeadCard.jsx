...
export default function LeadCard({ lead, now, noReplyHours, onOpen, onDragStart }) {
  ...
  return (
    <button type="button" draggable={typeof onDragStart === "function"} onDragStart={onDragStart ? (event) => onDragStart(event, lead) : undefined} onClick={() => onOpen(lead.id)} className="w-full rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-3.5 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-[var(--color-primary)]/40 hover:shadow-md active:translate-y-0">
