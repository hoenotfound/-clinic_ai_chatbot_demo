...
function normalizedLocation(lead) {
  if (!isRenovationDemo) return lead.branch;
...
function mapLead(lead) {
  ...
  return {
    ...
    appointment_status: lead.cancelled ? "cancelled" : lead.reschedule ? "reschedule" : lead.stage === "visited" ? "visited" : lead.stage === "confirmed" ? "confirmed" : lead.stage === "appointment" ? "requested" : "none",
...
function liveIntentLabel(lead) {
  if (!isRenovationDemo) return lead.bookingIntent ? "Appointment requested" : "No booking intent";
  if (lead.siteMeasurementIntent) return "Site measurement requested";
  if (lead.quotationIntent) return "Proper quotation requested";
  if (lead.humanRequest) return "Human/designer requested";
  if (lead.technicalHandoff) return "Technical handoff required";
  return "No high-intent action yet";
}
...
export default function Pipeline() {
  ...
  useEffect(() => {
    ...
    return () => { clearInterval(liveTimer); clearInterval(clockTimer); };
  }, []);

  const liveLead = useMemo(() => {
    if (!live?.customerMessageCount) return null;
    const lead = live.lead || {};
    let key = lead.bookingIntent ? "appointment" : lead.temperature === "hot" || lead.temperature === "warm" ? "interested" : "new";
    if (isRenovationDemo) {
      if (lead.siteMeasurementIntent) key = "appointment";
      else if (lead.quotationIntent || lead.humanRequest) key = "interested";
    }
    ...
    return {
      ...
      language: "LIVE",
      intent_summary: liveIntentLabel(lead),
    };
  }, [live]);
  ...
  const counts = useMemo(() => Object.fromEntries(CATEGORY_OPTIONS.map(([key]) => [key, leads.filter((lead) => matchesCategory(lead, key, now)).length])), [leads, now]);

  const branchCards = [
    { key: "all", label: `All ${industryProfile.terms.locationPlural.toLowerCase()}`, items: open },
    ...
    <div className="flex h-full min-w-0 flex-col overflow-hidden bg-[var(--color-bg)]">
      <header className="shrink-0 border-b border-[var(--color-border)] bg-[var(--color-surface)] px-4 py-4 sm:px-5 lg:px-6">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div><div className="flex items-center gap-2"><h1 className="font-display text-xl font-bold tracking-[-.02em] sm:text-2xl">Lead Pipeline</h1><span className="rounded-full bg-[var(--color-primary-light)] px-2 py-1 text-[9px] font-bold uppercase tracking-wide text-[var(--color-primary)]">Live</span><span className="rounded-full border border-[var(--color-border)] bg-[var(--color-bg)] px-2 py-1 text-[9px] font-semibold text-[var(--color-text-muted)]">Sample demo data</span></div><p className="mt-1 text-xs text-[var(--color-text-muted)]">Track every enquiry, next action and sales outcome in one place.</p></div>
          <div className="hidden gap-2 sm:flex" aria-label="Production-only pipeline controls"><button disabled title="Available in production" className="h-10 cursor-not-allowed rounded-xl border border-[var(--color-border)] bg-white px-3 text-xs font-semibold text-[var(--color-text-muted)] opacity-60">Manage stages · Production</button><button disabled title="Available in production" className="h-10 cursor-not-allowed rounded-xl bg-[var(--color-primary)] px-3 text-xs font-semibold text-white opacity-60">+ Add lead · Production</button></div>
        </div>
        <div className="mt-4 grid grid-cols-3 gap-2 sm:max-w-2xl"><Metric label="Active leads" value={open.length} detail="Open journeys" /><Metric label="Hot leads" value={open.filter((lead) => lead.temperature === "hot").length} detail="Priority follow-up" tone="danger" /><Metric label="Pipeline value" value={formatMoney(open.reduce((sum, lead) => sum + (Number(lead.estimated_value) || 0), 0)) || "RM 0"} detail="Estimated open value" /></div>
      </header>

      <div className="shrink-0 overflow-x-auto border-b border-[var(--color-border)] bg-white px-4 py-3 sm:px-5 lg:px-6"><div className="flex min-w-max gap-2">{branchCards.map((card) => <button key={card.key} onClick={() => setBranch(card.key)} className={`w-52 rounded-2xl border px-3.5 py-3 text-left transition ${branch === card.key ? "border-[var(--color-primary)] bg-[var(--color-primary-light)]" : "border-[var(--color-border)] bg-white hover:border-[var(--color-primary)]/30"}`}><div className="flex justify-between gap-2"><strong className="truncate text-xs">{card.label}</strong><b className="text-sm">{card.items.length}</b></div><p className="mt-1 text-[9px] text-[var(--color-text-muted)]">{card.items.filter((lead) => lead.temperature === "hot").length} hot · {card.items.filter((lead) => lead.appointment_status === "confirmed").length} confirmed</p></button>)}</div></div>

      <div className="shrink-0 border-b border-[var(--color-border)] bg-[var(--color-surface)] px-4 py-3 sm:px-5 lg:px-6"><div className="flex flex-col gap-2 xl:flex-row xl:items-center"><div className="relative w-full xl:w-64"><SearchIcon /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search leads…" className="h-10 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-bg)] pl-9 pr-3 text-xs outline-none focus:border-[var(--color-primary)] focus:ring-2 focus:ring-[var(--color-primary-light)]" /></div><div className="flex gap-1.5 overflow-x-auto pb-1">{CATEGORY_OPTIONS.map(([key, label]) => <button key={key} onClick={() => setCategory(key)} className={`flex h-9 shrink-0 items-center gap-1.5 rounded-full border px-3 text-[10px] font-semibold ${category === key ? "border-[var(--color-primary)] bg-[var(--color-primary)] text-white" : "border-[var(--color-border)] bg-white text-[var(--color-text-muted)] hover:border-[var(--color-primary)]/40"}`}>{label}<span className={`rounded-full px-1.5 py-0.5 text-[8px] ${category === key ? "bg-white/20" : "bg-[var(--color-bg)]"}`}>{counts[key]}</span></button>)}</div></div></div>
