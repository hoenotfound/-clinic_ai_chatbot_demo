...
function Breakdown({ tab, leads }) {
  if (industryProfile.key === "clinic") {
    const rows = tab === "Channel" ? [["WhatsApp", "68%", "19 won"], ["Instagram", "24%", "6 won"], ["Facebook", "8%", "2 won"]] : tab === "Treatment" ? [["HIFU", "38 leads", "7 won"], ["Pico Laser", "31 leads", "4 won"], ["Skin Booster", "22 leads", "4 won"], ["Botulinum Toxin", "17 leads", "2 won"]] : tab === "Owner" ? [["Mira", "51 leads", "8 won"], ["Sarah", "49 leads", "7 won"], ["Unassigned", "26 leads", "2 won"]] : tab === "Branch" ? [["Kuala Lumpur", "69 leads", "10 won"], ["Petaling Jaya", "57 leads", "7 won"]] : [["Meta Ads", "74 leads", "12 won"], ["Organic", "31 leads", "3 won"], ["Referral", "21 leads", "2 won"]];
    return <div className="divide-y divide-[var(--color-border)]">{rows.map((row) => <div key={row[0]} className="grid grid-cols-3 gap-2 py-3 text-xs"><strong>{row[0]}</strong><span className="text-[var(--color-text-muted)]">{row[1]}</span><span className="text-right font-semibold text-[var(--color-primary)]">{row[2]}</span></div>)}</div>;
  }

  const keyFor = (lead) => {
    if (tab === "Source") return lead.source;
    if (tab === "Campaign") return lead.source === "Meta Ads" ? "Demo Renovation Campaign" : "No campaign";
    if (tab === "Project") return lead.treatment;
    if (tab === "Area") return normalizedLocation(lead);
