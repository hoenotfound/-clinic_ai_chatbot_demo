const FORMAL_QUOTE_PATTERNS = [
  ...
  /\b(?:hantar|bagi|sediakan|buat|keluarkan)\b[^.!?]{0,40}\b(?:quotation|sebut\s+harga)\b|\b(?:quotation|sebut\s+harga)\s+(?:rasmi|final)\b/i,
];
...
const PAYMENT_DOCUMENT_PATTERNS = [
  ...
  /\b(?:link\s+bayaran|butiran\s+bayaran|cara\s+bayar\s+deposit|invois|resit|akaun\s+bank|butiran\s+bank)\b/i,
];
...
const DOCUMENT_REQUEST_PATTERNS = [
  ...
  /(?:发|發|给|給|send).{0,15}(?:目录|目錄|价目表|價目表|材料表|文件|PDF|保修资料|保修資料|brochure|catalogue)/i,
  /\b(?:hantar|share|bagi)\b[^.!?]{0,50}\b(?:brochure|katalog|senarai\s+harga|dokumen|pdf)\b/i,
];
...
const CAPABILITY_DISCLOSURE_PATTERNS = [
  ...
  /\b(?:i|we)\s+(?:do\s+not|don't)\s+(?:know|have)\s+(?:that|this|the)?\s*(?:information|detail|details|answer)?\b/i,
  /\b(?:i\s+am|i'm)\s+not\s+sure\b/i,
  /(?:作为|身为)\s*AI/i,
  /(?:我|这里|這裡|这个聊天|這個聊天|这个系统|這個系統).{0,10}(?:无法|無法|不能).{0,16}(?:发送|發送|提供|上传|上傳|安排|预约|預約|确认|確認|处理|處理|访问|存取)/i,
  /我(?:不知道|不确定|不確定|没有(?:这个|這個|相关|相關)?(?:资料|資料|信息|資訊)|沒有(?:這個|这个|相關|相关)?(?:資料|资料|資訊|信息))/i,
