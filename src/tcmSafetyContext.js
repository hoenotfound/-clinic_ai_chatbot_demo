...
const EXPLICIT_HUMAN_REQUEST = /(?:can|could|may)\s+i\s+(?:speak|talk|chat|see|meet)\s+(?:to|with)?\s*(?:(?:a|the)\s+)?(?:tcm\s+)?(?:human|staff|practitioner|doctor)|(?:i\s+)?(?:want|need)\s+(?:to\s+)?(?:speak|talk|chat|see|meet)\s+(?:to|with)?\s*(?:(?:a|the)\s+)?(?:tcm\s+)?(?:human|staff|practitioner|doctor)|(?:please\s+)?(?:connect|transfer|pass|put)\s+(?:me\s+)?(?:through\s+)?(?:to|with)\s+(?:(?:a|the)\s+)?(?:tcm\s+)?(?:human|staff|practitioner|doctor)|(?:boleh|nak|mahu)\s+(?:saya\s+)?(?:cakap|bercakap|jumpa|berjumpa)\s+(?:dengan\s+)?(?:pengamal(?:\s+TCM)?|doktor|staff|orang)|(?:nak|mahu)\s+(?:jumpa|cakap\s+dengan)\s+(?:pengamal(?:\s+TCM)?|doktor|staff)|真人|转人工|轉人工|找医师|找醫師|找医生|找醫生|想找中医师|想找中醫師|想跟中医师聊|想跟中醫師聊/i;
...
function isInformationalPractitionerMention(text) {
  ...
  return PRACTITIONER_INFO_QUERY.test(text);
}
