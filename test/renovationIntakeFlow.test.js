...
test("complete kitchen constraints produce tailored preliminary advice and let the provider answer naturally", () => {
  ...
  messages.push({
    role: "user",
    content: "The wall is clear except 2 plug points and a sink in the middle. No window.",
  });
  ...
  // The normal reply function intentionally returns null here so Gemini can use the
  // supplied site facts. Mock/provider failure paths use plan.adviceReply instead.
...
test("technical, complaint and unsupported-trade messages bypass intake instead of receiving the template", () => {
  const cases = [
    "Can I hack this wall for the cabinet?",
  ...
  for (const message of cases) {
    ...
    assert.equal(plan.appendAfterAnswer, null, message);
  }
});
